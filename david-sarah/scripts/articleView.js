...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {
      ...
      $('#articles').children('article').show();
      $('.template').hide();
...
articleView.setTeasers = function() {
  ...
  $('#articles').on('click', '.read-on', function() {
    ...
    $(this).prev('section').addClass('clicked');
    $(`.clicked *:nth-of-type(n+2)`).fadeIn(500);
    $(this).hide();
    $(this).siblings('.read-less').show();
  })
};

articleView.readLess = function() {
  $('#articles').on('click', '.read-less', function() {
    event.preventDefault();
    $(this).siblings('section').removeClass('clicked');
    $('.article-body *:nth-of-type(n+2)').hide();
